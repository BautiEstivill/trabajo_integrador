...
let pelibuscada2 = document.querySelector(".pelibuscada2")
...
fetch (`https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&query=${busqueda}`)
    ...
    .then(function(data){
        ...
        if (info.length === 0) {
            pelibuscada2.innerHTML = `No hay resultados para su busqueda de ${busqueda}`
            }
        else{pelibuscada2.innerHTML = ` hay resultados para su busqueda de ${busqueda}`}
